...
public class Board {
  ...
  public LinkedList<Card> getPlayereCards(int targetPlayer) {
    if (targetPlayer == 1) return playerOneCards;
    if (targetPlayer == 2) return playerTwoCards;
  ...
  public void updateCardPositions(LinkedList<Card> list) {
    ...
    for (int i = 0; i < +list.size(); i++) {
      list.get(i)
          .getLocation()
          .setLocation(
              roundBoardHorizontalPositionByIndex(startingIndex + i + 1 - remainder),
              location.y + height * 3 / 4);
    }
  }
